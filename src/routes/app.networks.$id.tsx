  ...
  const { data: packages, isLoading: pkgsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("packages")
        .select(
          "id, name, price, data_size, speed, validity, allowed_time, description, color, sort_order, is_active",
        )
  ...
  return (
    <>
      <div className="mb-6">
        <Link
          to="/app/networks"
          className="text-xs text-muted-foreground inline-flex items-center gap-1 mb-3"
        >
            ...
            isAdmin ? (
              ...
              >
                ...
                <DialogContent className="max-w-md rounded-3xl" dir="rtl">
                  ...
                  <PackageForm
                    initial={editing}
                    onSubmit={(v) => savePkg.mutate(v)}
                    busy={savePkg.isPending}
                  />
                </DialogContent>
              </Dialog>
            ) : undefined
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {packages?.map((p) => {
          ...
          return (
            <Card
              key={p.id}
              className="card-elegant card-elegant-hover border-0 overflow-hidden slide-up p-4 space-y-4"
            >
              ...
              <div className="flex justify-end">
                <div className="rounded-xl border bg-muted/70 px-3 py-1.5 text-sm text-muted-foreground">
                  ID: {packageCode}
                </div>
              </div>
                ...
                {isAdmin ? (
                  ...
                  <>
                    <Button
                      ...
                      onClick={() => setRequestPkg(p)}
                      variant="outline"
                      className="flex-1 rounded-xl h-11 border-primary text-primary font-semibold"
                    >
          ...
          {requestPkg && (
            <div className="mt-4 space-y-4">
              <div
                className="rounded-2xl p-5 text-white"
                style={{
                  background: `linear-gradient(135deg, ${requestPkg.color}, ${requestPkg.color}dd)`,
                }}
              >
...
function PackageInfo({
  ...
  return (
    <div className="rounded-xl bg-muted/60 border p-3 min-h-24 flex flex-col items-center justify-center gap-1">
      ...
      <div className="font-extrabold text-lg leading-tight break-words max-w-full text-center">
        {value}
      </div>
    ...
    <div className="mt-4 space-y-3 pb-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button
          variant="outline"
          className="rounded-xl"
          onClick={() => copy(fullText, "البيانات كاملة")}
        >
        ...
        <Button variant="outline" className="rounded-xl" onClick={whatsapp}>
