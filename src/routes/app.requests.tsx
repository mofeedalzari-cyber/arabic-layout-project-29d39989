...
function RequestList({ status, isAdmin }: { status: string; isAdmin: boolean }) {
  ...
  const pay = useMutation({
    ...
    onError: (e: Error) => {
      const msg = e.message.includes("EXCEEDS_TOTAL")
        ? "المبلغ يتجاوز المتبقي"
        : e.message.includes("INVALID_AMOUNT")
          ? "أدخل مبلغاً صحيحاً"
          : e.message.includes("NOT_APPROVED")
            ? "الطلب غير معتمد"
            : e.message;
      toast.error(msg);
    },
            ...
            <Card key={r.id} className="card-elegant border-0 p-4">
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <div className="min-w-0 flex-1 space-y-1 text-sm">
                  ...
                  <div>
                    <span className="text-muted-foreground text-xs">الشبكة: </span>
                    {r.network_name}
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-muted-foreground text-xs">عدد الكروت: </span>
                    <b>{qty}</b>
                  </div>
                  <div>
                    <span className="text-muted-foreground text-xs">القيمة الإجمالية: </span>
                    <b className="text-primary">{fmtMoney(total)}</b>
                  </div>
                  <div>
                  ...
                  {r.notes && (
                    <div className="text-xs bg-muted/50 rounded-lg p-2 mt-1">📝 {r.notes}</div>
                  )}
                  {r.reject_reason && (
              ...
              <div className="flex items-center gap-2 flex-wrap pt-3 border-t border-border/50">
                <div
                  className={`inline-flex items-center gap-1.5 rounded-lg px-3 h-9 text-xs font-semibold ${
          ...
          {rejectFor && (
            <div className="space-y-3">
              <div className="text-sm text-muted-foreground">
                طلب <b>{display(rejectFor.agent_username)}</b> — {rejectFor.package_name} (
                {rejectFor.quantity})
              </div>
              <div>
                ...
                <div className="space-y-3">
                  <div className="text-sm">
                    <div className="text-muted-foreground">
                      المندوب: <b className="text-foreground">{display(payFor.agent_username)}</b>
                    </div>
                    <div className="text-muted-foreground">
                      الطلب: <b className="text-foreground">{payFor.package_name}</b>
                    </div>
                  </div>
