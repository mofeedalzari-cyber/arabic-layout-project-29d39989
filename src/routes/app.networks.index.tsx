  ...
  const { data: networks, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("networks")
        .select(
          "id, name, description, currency, primary_color, secondary_color, is_active, created_at",
        )
  ...
  const { data: counts } = useQuery({
    ...
    queryFn: async () => {
      ...
      cards.data?.forEach((c) => {
        const g = get(c.network_id);
        if (c.status === "AVAILABLE") g.avail++;
        else g.sold++;
      });
      sales.data?.forEach((s) => {
        get(s.network_id).value += Number(s.price);
      });
      return m;
    },
    enabled: canCreate,
  ...
  const saveMutation = useMutation({
    mutationFn: async (form: NetForm) => {
      if (editing) {
        ...
        const { error } = await supabase.rpc("superadmin_create_network", {
          _name: form.name,
          ...
          canCreate ? (
            ...
            >
              ...
              <DialogContent className="max-w-md rounded-3xl" dir="rtl">
                <DialogHeader>
                  <DialogTitle>{editing ? "تعديل شبكة" : "شبكة جديدة"}</DialogTitle>
                </DialogHeader>
                <NetworkForm
                  initial={editing}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {networks?.map((n) => {
            ...
            return (
              <Card
                key={n.id}
                className="card-elegant card-elegant-hover overflow-hidden border-0 slide-up"
              >
                <Link to="/app/networks/$id" params={{ id: n.id }}>
                  <div
                    className="h-28 relative"
                    style={{
                      background: `linear-gradient(135deg, ${n.primary_color}, ${n.secondary_color})`,
                    }}
                  >
                    ...
                    {!n.is_active && (
                      <div className="absolute top-2 right-2 bg-black/40 text-white text-[11px] px-2 py-0.5 rounded-full">
                        موقوفة
                      </div>
                    )}
                  </div>
                  ...
                  {isAdmin && (
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (confirm(`حذف شبكة "${n.name}"؟ سيتم حذف الباقات والكروت المرتبطة.`))
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {Array.from({ length: 6 }).map((_, i) => (
        <Card key={i} className="card-elegant border-0 h-56 animate-pulse" />
      ))}
    </div>
  );
}
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <Label className="text-xs">لون أساسي</Label>
          <Input
            type="color"
        ...
        <div>
          <Label className="text-xs">لون ثانوي</Label>
          <Input
            type="color"
