...
function ManageCardsPageInner() {
    ...
    queryFn: async () => {
      ...
      return (data ?? []).map((p: any) => ({
        ...
        name:
          ...
          displayPhone(p.phone as string | null, p.username as string | null) ||
  ...
  const { data: cards, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      const ids = Array.from(
        new Set(rows.flatMap((r) => [r.assigned_to, r.sold_to]).filter(Boolean) as string[]),
      );
      if (ids.length) {
        ...
        const nameMap = new Map(
          (profs ?? []).map((p: any) => [
            p.id,
            (p.full_name as string | null) || displayPhone(p.phone, p.username),
          ]),
        );
        rows.forEach((r) => {
  ...
  const del = useMutation({
    mutationFn: async () => {
      ...
      if (otherIds.length) {
        ...
        const { data, error } = await supabase.rpc("admin_delete_cards", {
          _ids: otherIds,
          _force: forceDelete,
        });
        if (error) throw error;
  ...
  const delOldSold = useMutation({
    mutationFn: async () => {
      ...
      const ids = cards
        .filter(
          (c) =>
            (c.status === "SOLD" || c.status === "ASSIGNED") && new Date(c.created_at) < cutoff,
        )
        .map((c) => c.id);
  ...
  const delOne = useMutation({
    mutationFn: async (id: string) => {
      ...
      const { error } = await supabase.rpc("admin_delete_cards", {
        _ids: [id],
        _force: extendedDelete || Boolean(refs?.length) || isSold,
      });
      if (error) throw error;
  ...
  async function printAssigned() {
    ...
    if (!chosen.length) {
      toast.error("لا توجد كروت مسحوبة للطباعة");
      return;
    }
    ...
    try {
      ...
      console.error("[printAssigned] failed:", err);
  ...
  async function printAvailable() {
    ...
    const chosen = selected.size ? src.filter((c) => selected.has(c.id)) : src;
    if (!chosen.length) {
      toast.error("لا توجد كروت متاحة للطباعة");
      return;
    }
    const netName = networks?.find((n) => n.id === networkId)?.name ?? "";
    try {
  ...
  const selectedAvailableIds = (cards ?? [])
    .filter((c) => c.status === "AVAILABLE" && selected.has(c.id))
    .map((c) => c.id);
      ...
      <Card className="card-elegant border-0 p-4 mb-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground mt-1">
              يتم عرض مناديب الشبكة المحددة فقط.
            </p>
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 border-t">
          <span className="text-xs">
            المحدد: <b className="text-primary">{selected.size}</b> كرت
          </span>
          <div className="mr-auto flex items-center gap-2 text-xs">
            ...
            <Label htmlFor="ext-del" className="cursor-pointer">
              وضع الحذف الموسع
            </Label>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent dir="rtl">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    سيتم حذف الكروت المحددة. إذا كان ضمنها كروت مباعة فسيتم حذف عملية البيع المرتبطة
                    بها حتى ينقص حساب المندوب مباشرة.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
          ...
          <Button
            ...
            disabled={!cards?.some((c) => c.status === "SOLD")}
          >
            تحديد كل المباع
          </Button>
          <Button
            ...
            disabled={!cards?.some((c) => c.status === "ASSIGNED")}
          >
            تحديد كل المسحوب
          </Button>
          <Button
            variant="outline"
            className="rounded-lg h-9 text-emerald-600 border-emerald-500/40"
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              ...
              >
                <Trash2 className="h-4 w-4 ml-1" />
                حذف المتاح{selectedAvailableIds.length ? ` (${selectedAvailableIds.length})` : ""}
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent dir="rtl">
              <AlertDialogHeader>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent dir="rtl">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  سيتم حذف كل الكروت المسحوبة أو المباعة والتي مضى على إنشائها أكثر من 30 يومًا.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>إلغاء</AlertDialogCancel>
                <AlertDialogAction
                  onClick={() => delOldSold.mutate()}
                  className="bg-destructive text-destructive-foreground"
                >
                  تأكيد
                </AlertDialogAction>
            ...
            <table className="w-full text-sm min-w-[720px]">
                ...
                <tr>
                  <th className="p-3 w-10">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => (v ? selectPage() : unselectPage())}
                    />
                  </th>
              ...
              <tbody className="divide-y">
                {pageRows.map((c, idx) => {
                  const agentName =
                    c.status === "ASSIGNED"
                      ? c.assigned_full_name || displayPhone(null, c.assigned_username)
                      : c.status === "SOLD"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-xs">
                        {agentName || <span className="text-muted-foreground">—</span>}
                      </td>
