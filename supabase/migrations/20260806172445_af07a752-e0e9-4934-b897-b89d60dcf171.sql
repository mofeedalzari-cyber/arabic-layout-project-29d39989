DROP FUNCTION IF EXISTS public.superadmin_networks();
...
BEGIN
  ...
  SELECT n.id, n.name, n.currency, n.is_active, n.owner_id,
         ...
         (SELECT COALESCE(sum(total_value),0) FROM card_requests WHERE network_id=n.id AND status='approved'),
         (SELECT COALESCE(sum(paid_amount),0) FROM card_requests WHERE network_id=n.id AND status='approved'),
         (SELECT COALESCE(sum(total_value - paid_amount),0) FROM card_requests WHERE network_id=n.id AND status='approved'),
         n.created_at
