...
DECLARE v_uid uuid := auth.uid(); v_username text; v_pkg record; v_net record; v_id uuid;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED'; END IF;
  IF NOT public.is_active_user(v_uid) THEN RAISE EXCEPTION 'ACCOUNT_INACTIVE'; END IF;
  ...
  RETURN v_id;
END; $$;
GRANT EXECUTE ON FUNCTION public.request_cards(uuid, int, text) TO authenticated;
...
END; $$;
GRANT EXECUTE ON FUNCTION public.admin_list_cards(uuid, uuid, uuid, text, int) TO authenticated;
