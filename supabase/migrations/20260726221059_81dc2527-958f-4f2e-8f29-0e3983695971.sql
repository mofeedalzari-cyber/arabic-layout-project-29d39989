...
BEGIN
  ...
  -- Non-admin (agent) path: only buyer_name and customer_id may change
...
DROP TRIGGER IF EXISTS trg_restrict_agent_sales_update ON public.sales;
CREATE TRIGGER trg_restrict_agent_sales_update
BEFORE UPDATE ON public.sales
FOR EACH ROW EXECUTE FUNCTION public.restrict_agent_sales_update();
