...
BEGIN
  ...
  LOOP
    ...
    -- move the card and its sale record to the new agent
    ...
    -- reduce old agent debt (one card worth) on his approved requests
    IF v_card.sold_to IS NOT NULL THEN
      FOR v_req IN
        ...
        WHERE network_id = v_net
          ...
          AND package_id = v_card.package_id
          AND status = 'APPROVED'
      ...
      LOOP
        ...
        v_reduce := LEAST(COALESCE(NULLIF(v_req.unit_price,0), v_price), v_remaining);
    ...
    -- add the amount as debt on the new agent
...
GRANT EXECUTE ON FUNCTION public.admin_transfer_sold_cards(uuid[], uuid) TO authenticated;