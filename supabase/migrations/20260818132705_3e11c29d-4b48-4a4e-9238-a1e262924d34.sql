...
AS $function$
BEGIN
  PERFORM public.delete_sale(_sale_id, false);
END;
$function$;
...
BEGIN
  ...
  IF _force THEN
    WITH deleted_customer_sales AS (
    ...
    SELECT count(*)::integer INTO v_deleted_sales FROM deleted_customer_sales;
...
GRANT EXECUTE ON FUNCTION public.delete_sale(uuid, boolean) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_delete_cards(uuid[], boolean) TO authenticated, service_role;